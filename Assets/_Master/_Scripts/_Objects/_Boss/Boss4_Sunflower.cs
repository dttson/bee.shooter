...
public class Boss4_Sunflower : BossBase
{
    ...
    protected override void startMoving()
    {
        ...
        m_CoroutineShooting = StartCoroutine(coroutineRushAttack(m_Data.attackRate, m_SkillData.speed,
            m_SkillData.subAttackCount));
    }

    private IEnumerator coroutineRushAttack(float attackRate, float rushSpeed, int subAttackCount)
    {
        ...
        while (true)
        {
            ...
            float y = m_ScreenRect.yMin + bossSize.y / 2;
            float z = playerPos.z;
            ...
            // third hit is center
            attackPositions.Add(new Vector3(m_ScreenRect.center.x, y, playerPos.z));
            
            // play sound for first attack
            // playSoundSkill();
            ...
            for (int i = 0; i < subAttackCount; i++)
            {
                ...
                if (i < subAttackCount - 1)
                {
                    m_Sequence.Append(transform.DOMove(attackStartPos, rushDuration * 2).OnComplete(() =>
                    {
                        ...
                        // // play sound for next sub attack
                        // playSoundSkill();
                    }));
                }
                else
